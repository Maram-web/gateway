pipeline {
    ...
    environment {
        IMAGE_NAME = "marammanai/gateway-service:latest"
    ...
    stages {
        stage('Checkout') {
            steps {
                git branch: 'main', url: 'https://github.com/Maram-web/gateway.git'
            }
        }
    ...
    post {
        success { echo "✅ gateway-service deployed!" }
        failure { echo "❌ gateway-service failed!" }
    }
}
